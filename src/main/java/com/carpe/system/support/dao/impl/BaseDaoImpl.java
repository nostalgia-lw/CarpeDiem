package com.carpe.system.support.dao.impl;

import com.carpe.system.support.dao.BaseDao;
import com.carpe.system.support.hibernate.HibernateHandler;
 ...
 */
@Repository
public class BaseDaoImpl<T> extends HibernateSession implements BaseDao<T> {
	...
	public Object save(Object entity) {
		return (Object) getSession().save(entity);
	}
	...
	@SuppressWarnings({ "unchecked", "hiding" })
	public <T> Pagination<T> findPagination(CharSequence queryString, Long pageIndex, Long pageSize, Object... params) {
		Query query = getSession().createQuery(queryString.toString());

		if ((pageSize > 0) && (pageIndex > 0)) {
			query.setFirstResult((pageIndex.intValue() < 2) ? 0 : (pageIndex.intValue() - 1) * pageSize.intValue());
	...
	@SuppressWarnings({ "unchecked", "hiding" })
	public <T> Pagination<T> findPagination(CharSequence queryString, Long pageIndex, Long pageSize, Map<String, Object> params) {
		Query query = getSession().createQuery(queryString.toString());
	...
	@SuppressWarnings({ "unchecked", "hiding" })
	public <T> Pagination<T> findPagination(CharSequence queryString, CharSequence countString, Long pageIndex, Long pageSize, Object... params) {
		Query query = getSession().createQuery(queryString.toString());
	...
	@SuppressWarnings({ "unchecked", "hiding" })
	public <T> Pagination<T> findPagination(CharSequence queryString, CharSequence countString, Long pageIndex, Long pageSize, Map<String, Object> params) {
		Query query = getSession().createQuery(queryString.toString());
	...
	public <T> Pagination<T> findSqlPagination(CharSequence queryString, final CharSequence countString, final Map<String, Object> params, Long pageIndex, Long pageSize) {
		...
		if ((pageSize > 0) && (pageIndex > 0)) {
			Object rowsCountObj = executeQuery(new HibernateHandler() {
			...
			rowsCount = Long.parseLong(rowsCountObj.toString());
		}
	...
	@Override
	public <V> List<V> findListByMax(final CharSequence queryString, final int maxResults, final Map<String, Object> params) {
		@SuppressWarnings({ "unchecked", "serial" })
		List<V> list = (List<V>) executeQuery(new HibernateHandler() {
			@Override
			public List<V> doInHibernate(Session paramSession) {
	...
	public class DmlHelper {
		...
		public DmlHelper addDayRange(String fieldName, String minTime, String maxTime) {
			...
			if (hasText(minTime) && hasText(maxTime)) {
				try {
		...
		public <D> Pagination<D> findPagin(Long pageIndex, Long pageSize) {
